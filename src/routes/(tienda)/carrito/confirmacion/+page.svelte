<script lang="ts">
	...
	async function checkStatus(ref: string) {
		try {
			...
			if (!res.ok) return;
			const data = (await res.json()) as { status: 'pending' | 'approved' | 'declined'; wooOrderId: number | null };
...
<svelte:head>
	<title>Confirmación de pago - Coco's</title>
</svelte:head>

<div class="flex min-h-[70vh] items-center justify-center px-4">
	<div class="max-w-md text-center">
		{#if status === 'pending'}
			<div class="mx-auto mb-6 h-10 w-10 animate-spin rounded-full border-4 border-[#d1d1d1] border-t-[#262635]"></div>
			<h1 class="font-['Volkhov',serif] text-2xl font-bold text-[#262635]">Confirmando tu pago...</h1>
		...
		{:else if status === 'approved'}
			...
			<p class="mt-2 text-sm text-[#6a6a6a]">
				{#if wooOrderId}Tu pedido #{wooOrderId} fue registrado correctamente.{:else}Tu pedido fue registrado correctamente.{/if}
			</p>
			<a
				href={store.basePath || '/'}
				class="mt-6 inline-block rounded-xl bg-[#2f3c4f] px-6 py-3 text-sm font-semibold uppercase tracking-[0.08em] text-white transition-colors hover:bg-[#253243]"
			>
				Volver a la tienda
			</a>
		{:else if status === 'declined'}
			<h1 class="font-['Volkhov',serif] text-2xl font-bold text-[#262635]">El pago no fue aprobado</h1>
		...
		{:else if status === 'timeout'}
			<h1 class="font-['Volkhov',serif] text-2xl font-bold text-[#262635]">Confirmación pendiente</h1>
			<p class="mt-2 text-sm text-[#6a6a6a]">
				Wompi está tardando en confirmar tu pago. Te avisaremos por correo apenas se procese.
			</p>
		{:else}
			<h1 class="font-['Volkhov',serif] text-2xl font-bold text-[#262635]">No encontramos ese pago</h1>
